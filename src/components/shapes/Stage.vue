<template>
  <g :transform="transform" :style="style">
    ...
    <text
      class="level-number"
      :x="positionText(level.level, 2.5)"
      y="-4"
    >{{level.level}}</text>
    ...
    <text
      ...
      class="level-score"
      :x="positionText(level.score, 1.6)"
      y="10"
    >{{level.score}}</text>
    <path
      v-if="!level.isAvailable"
  ...
  computed: {
    transform() {
      return `translate(${120 +
        60 * (Math.floor((this.level.level - 1) / 10) % 2) +
        ((this.level.level - 1) % 10) * 120}, ${90 +
        Math.floor((this.level.level - 1) / 10) * 104})scale(4)`;
    },
    fill() {
